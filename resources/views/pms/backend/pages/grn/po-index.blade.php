                        ...
                        <table class="table table-striped table-bordered table-head datatable-exportable" data-table-name="{{ $title }}" cellspacing="0" width="100%" id="dataTable">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{isset($values->Unit->hr_unit_short_name)?$values->Unit->hr_unit_short_name:''}}</td>
                                    <td>{{date('d-m-Y',strtotime($values->po_date))}}</td>
                                    <td>{{isset($values->relQuotation->relSuppliers)?$values->relQuotation->relSuppliers->name:''}}</td>
                                    ...
                                    <td>{{isset($values->relQuotation->reference_no)?$values->relQuotation->reference_no:''}}</td>

                                    <td class="text-center">{{$values->relPurchaseOrderItems->sum('qty')}}</td>
                                    <td class="text-center">{{$values->total_grn_qty}}</td>

                                    <td class="text-right">{{systemMoneyFormat($values->total_price)}}</td>
                                    <td class="text-right">{{systemMoneyFormat($values->discount)}}</td>
                                    <td class="text-right">{{systemMoneyFormat($values->vat)}}</td>
                                    <td class="text-right">{{systemMoneyFormat($values->gross_price)}}</td>
                                    <td>    
                                        {!! printBarcode($values->reference_no) !!}
                                    </td>
                                    ...
                                    <td class="text-center">
                                        ...
                                        @elseif($values->relPurchaseOrderItems->sum('qty')>$values->total_grn_qty)
                                            ...
                                            <a href="{{ route('pms.grn.grn-list.create',$values->id) }}" data-toggle="tooltip" title="Click here to generate Gate-In">
