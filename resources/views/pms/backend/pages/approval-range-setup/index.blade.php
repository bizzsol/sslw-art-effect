                    ...
                    <table  id="dataTable" class="table table-striped table-bordered table-head" border="1">
                        ...
                        @foreach($ranges as $key => $range)
                            <tr>
                                ...
                                <td class="text-center">{{ systemMoneyFormat($range->min_amount) }}</td>
                                <td class="text-center">{{ systemMoneyFormat($range->max_amount) }}</td>
                                <td>
                                    @foreach($range->relUsers as $rangeUser)
                                        <span>{{ $rangeUser->name }}</span>,
                                    @endforeach
                                </td>
                                <td class="text-center">
                                    @can('range-setup-edit')
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="" method="post">
                    @csrf
                    <p class="mb-1 font-weight-bold"><label for="minAmount"><strong>{{ __('From Amount') }}<span class="text-danger">&nbsp;*</span></strong></label> {!! $errors->has('min_amount')? '<span class="text-danger text-capitalize">'. $errors->first('min_amount').'</span>':'' !!}</p>
                    ...
                    <p class="mb-1 font-weight-bold"><label for="maxAmount"><strong>{{ __('To Amount') }}<span class="text-danger">&nbsp;*</span></strong></label> {!! $errors->has('max_amount')? '<span class="text-danger text-capitalize">'. $errors->first('max_amount').'</span>':'' !!}</p>
                    <div class="input-group input-group-md mb-3 d-">
                    ...
                    <p class="mb-1 font-weight-bold"><label for="role"><strong>{{ __('Roles') }}<span class="text-danger">&nbsp;*</span></strong></label> {!! $errors->has('role')? '<span class="text-danger text-capitalize">'. $errors->first('role').'</span>':'' !!}</p>
                    ...
                    <p class="mb-1 font-weight-bold"><label for="users"><strong>{{ __('Users') }}<span class="text-danger">&nbsp;*</span></strong></label> {!! $errors->has('users')? '<span class="text-danger text-capitalize">'. $errors->first('users').'</span>':'' !!}</p>
                    <div class="select-search-group input-group input-group-md mb-3 d-">
