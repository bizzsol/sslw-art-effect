							...
							<div class="row">
								...
								<div class="col-md-4">
									...
									<div class="select-search-group input-group input-group-md mb-3 d-">
										<select name="parent_id" id="parent" class="form-control" onchange="updateFinanceSection()">
											<option value="{{ null }}">{{ __('Select One') }}</option>
											@foreach($categories as $key => $cat)
											@if(!isset($cat->category->id))
											<option value="{{ $cat->id }}" data-departments="{{ $cat->departmentsList->pluck('hr_department_id')->implode(',') }}" {{ ($cat->id == $category->category->id)?'selected':'' }} is_fixed_asset="{{ $cat->is_fixed_asset }}" is_cwip="{{ $cat->is_cwip }}" data-product-type="{{ $cat->is_fixed_asset == 1 ? 'fixed_asset' : ($cat->is_cwip == 1 ? 'cwip' : 'products') }}" data-service="{{ $cat->is_service }}">{{ $cat->name.' ('.$cat->code.' )' }} ({{ $cat->is_fixed_asset == 1 ? 'Fixed Asset' : ($cat->is_cwip == 1 ? 'CWIP' : 'Products') }})</option>
											@endif
											@endforeach
										</select>
									</div>
