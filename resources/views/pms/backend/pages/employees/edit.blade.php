        ...
        <div class="page-content">
            <div class="">
                <div class="panel panel-info">
                    <form  method="post" action="{{ route('pms.employees.update', $employee->as_id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="panel-body">
                          ...
                          <div class="form-group row">
                                <div class="col-md-2">
                                  <label for="associate_id"><strong>Employee UID <span class="text-danger">&nbsp;*</span> </strong></label>
                                  <input type="text" class="form-control" name="associate_id" value="{{ old('associate_id', $employee->associate_id) }}" id="associate_id" required>
                                </div>
                                <div class="col-md-4">
                                  <label for="as_name"><strong>Employee Name <span class="text-danger">&nbsp;*</span> </strong></label>
                                  <input type="text" class="form-control" name="as_name" value="{{ old('as_name', $employee->as_name) }}" id="as_name" required>
                                </div>
                                <div class="col-md-3">
                                  <label for="as_designation_id"><strong>Designation <span class="text-danger">&nbsp;*</span> </strong></label>
                                  <select name="as_designation_id" id="as_designation_id" class="form-control" required>
                                    @if(isset($designations[0]))
                                    @foreach($designations as $key => $designation)
                                    <option value="{{ $designation->hr_designation_id }}" {{ $designation->hr_designation_id == $employee->as_designation_id ? 'selected' : '' }}>{{ $designation->hr_designation_name }}</option>
                                    @endforeach
                                    @endif
                                  </select>
                                </div>
                                <div class="col-md-3">
                                  <label for="as_doj"><strong>Joining Date <span class="text-danger">&nbsp;*</span> </strong></label>
                                  <input type="date" class="form-control" name="as_doj" value="{{ old('as_doj', date('Y-m-d', strtotime($employee->as_doj))) }}" id="as_doj" required>
                                </div>
                          </div>
                          <div class="form-group row">
                                <div class="col-md-3">
                                  <label for="as_dob"><strong>Date of Birth <span class="text-danger">&nbsp;*</span> </strong></label>
                                  <input type="date" class="form-control" name="as_dob" value="{{ old('as_dob', date('Y-m-d', strtotime($employee->as_dob))) }}" id="as_dob" required>
                                </div>
                                <div class="col-md-2">
                                  <label for="as_gender"><strong>Gender <span class="text-danger">&nbsp;*</span> </strong></label>
                                  <select name="as_gender" id="as_gender" class="form-control">
                                    <option {{ $employee->as_gender == "Male" ? 'selected' : '' }}>Male</option>
                                    <option {{ $employee->as_gender == "Female" ? 'selected' : '' }}>Female</option>
                                  </select>
                                </div>
                                <div class="col-md-3">
                                  <label for="as_contact"><strong>Contact <span class="text-danger">&nbsp;*</span> </strong></label>
                          ...
                          <button type="submit" class="mt-2 btn btn-success rounded">
                              <i class="la la-check"></i>&nbsp;{{ __('Update Employee') }}
                          </button>
                        </div>
                    </form>
                </div>
